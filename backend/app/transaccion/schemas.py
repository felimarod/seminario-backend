...
class TransaccionBase(BaseModel):
    ...
    estado_transaccion: str = Field(
        ..., max_length=100, description="Estado de la transaccion"
    )
...
class TransaccionUpdate(BaseModel):
    ...
    falla_servicio: Optional[str] = Field(
        ..., max_length=255, description="Falla del servicio"
    )
    id_tipo_transaccion: Optional[int] = Field(
        ..., description="ID del tipo de transaccion"
    )
