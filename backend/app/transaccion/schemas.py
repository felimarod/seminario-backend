...
class TransaccionBase(BaseModel):
    ...
    fecha_inicio_transaccion: Optional[datetime] = Field(
        None, description="fecha y hora de inicio en formato AAAA-MM-DD HH:MM"
    )
...
class TransaccionCreate(TransaccionBase):
    ...
    fecha_inicio_transaccion: datetime = Field(
        ..., description="fecha y hora de inicio en formato AAAA-MM-DD HH:MM"
    )
    fecha_fin_transaccion: datetime = Field(
        ..., description="fecha y hora de finalización en formato AAAA-MM-DD HH:MM"
    )
...
class TransaccionResponse(BaseModel):
    ...
    def from_transaccion_db(cls, transaccion_db: Transaccion):
        ...
        estado = sorted(
            transaccion_db.historial, key=lambda h: h.fecha_cambio, reverse=True
        )[0].estado
        data["estado_actual"] = {
...
class Filtros(BaseModel):
    ...
    ventana_tiempo_inicio: Optional[datetime] = Field(
        default=None,
        description="inicio de ventana temporal en formato AAAA-MM-DD HH:MM",
    )
    ventana_tiempo_fin: Optional[datetime] = Field(
        default=None, description="fin de ventana temporal en formato AAAA-MM-DD HH:MM"
    )
    ventana_atributo: Optional[str] = Field(
        default=None, description="atributo a buscar en la ventana de tiempo"
    )
    id_usuario: Optional[int] = Field(
        default=None, description="id del usuario que solicita el recurso"
    )
    id_recurso: Optional[str] = Field(
        default=None, description="id recurso del recurso solicitado"
    )
    id_empleado_responsable: Optional[int] = Field(
        default=None, description="id del empleado que da/recibe el recurso"
    )
    estado_transaccion: Optional[List[int]] = Field(
        default=None, description="listado de id del estado de la transaccion"
    )

    id_tipo_recurso: Optional[int] = Field(
        default=None, description="id del tipo de recurso solicitado"
    )
    id_unidad: Optional[int] = Field(
        default=None, description="id de la unidad al que pertence el recurso"
    )
...
class PrestamoIN(DevolucionIN):
    ...
    password_user: str = Field(
        ..., description="contraseña del usuario que solicita el recurso"
    )
