...
def get_transaccions(
    ...
    transacciones = TransaccionSelectors.get_filter(
        db, filtros=filtros, skip=skip, limit=limit
    )
    if filtros.estado_transaccion:
        return [
            TransaccionResponse.from_transaccion_db(transaccion)
            for transaccion in transacciones
            if transaccion.lastHistorial().id_estado_transaccion
            in filtros.estado_transaccion
        ]
    return [
        TransaccionResponse.from_transaccion_db(transaccion)
        for transaccion in transacciones
...
def create_transaccion(
    request: Request, transaccion_data: TransaccionCreate, db: Session = Depends(get_db)
):
    """Crea un nuevo transaccion."""
    user = request.state.user
    if user["tipo"] not in (3, 4):
        print(f"user tipo: {user['tipo']}")
        raise HTTPException(
            status_code=403, detail="No tienes permiso para crear una reserva"
        )
    if user["tipo"] == 3:
    ...
    if user["tipo"] == 4:
        if transaccion_data.fecha_inicio_transaccion is None:
            raise HTTPException(
                status_code=403,
                detail="Ingresa una fecha de inicio para realizar la reserva",
            )
        if transaccion_data.id_empleado_responsable is not None:
            raise HTTPException(
                status_code=403,
                detail="No tienes permiso para asignar un empleado responsable",
            )
...
def reserva_to_prestamo(
    request: Request, body: PrestamoIN, db: Session = Depends(get_db)
):
    ...
    if user["tipo"] != 3:
        raise HTTPException(
            status_code=403,
            detail="No tienes permiso para actualizar una reserva a prestamo",
        )
    transaccion = TransaccionService.prestar(
        db, body.id_transaccion, int(user["id"]), body.password_user
    )
...
def prestamo_to_devolucion(
    request: Request, body: DevolucionIN, db: Session = Depends(get_db)
):
    """Actualiza estado de transaccion de reserva a prestamo."""
    user = request.state.user
    if user["tipo"] != 3:
...
def update_transaccion(
    ...
    if user["tipo"] != 4:
        raise HTTPException(
            status_code=403, detail="No tienes permiso para modificar una reserva"
        )
    transaccion_data.id_usuario = user["id"]
    return TransaccionResponse.from_transaccion_db(
        TransaccionService.cambiar_fechas(db, transaccion_data)
    )
